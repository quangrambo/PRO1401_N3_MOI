...
public class NhanVienVMRepository {
    ...
    public List<NhanVienVM> getAllNVVM() {
        String sql = "select nv.MA,nv.HOTEN,nv.GIOTINH,nv.SDT,nv.CCCD,nv.NGAYSINH,nv.DIACHI,nv.EMAIL,nv.CHUCVU,tk.TENDANGNHAP,tk.MATKHAU,nv.TRANGTHAI from NHAN_VIEN nv  join TAI_KHOAN tk on nv.ID = tk.ID_NV";
        ResultSet rs = JDBCHelper.excuteQuery(sql);
        return getNhanVienFromResultSet(rs);
    }
    public List<NhanVienVM> getAllNVByTrangThai() {
        String sql = "select nv.MA,nv.HOTEN,nv.GIOTINH,nv.SDT,nv.CCCD,nv.NGAYSINH,nv.DIACHI,nv.EMAIL,nv.CHUCVU,tk.TENDANGNHAP,tk.MATKHAU,nv.TRANGTHAI from NHAN_VIEN nv  join TAI_KHOAN tk on nv.ID = tk.ID_NV where nv.TRANGTHAI = 1";
        ResultSet rs = JDBCHelper.excuteQuery(sql);
        return getNhanVienFromResultSet(rs);
    }
