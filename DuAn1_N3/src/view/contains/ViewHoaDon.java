    ...
    public void loadTable(List<HoaDonView> list){
     ...
     public void loadTableCTHD(List<SanPhamHDViewModel> list){
        ...
        for (SanPhamHDViewModel p : list) {
            ((DefaultTableModel) tblGioHang.getModel()).addRow(new Object[]{p.getTenSp(),p.getLoaiSanPham(),p.getThuongHieu(),p.getMauSac(),p.getKichCo(),p.getChatLieu(),p.getSoLuong(),p.getDonGia()});
        }
...
;
                ...
                loadTable(list);
                 ...
                 JOptionPane.showMessageDialog(new JFrame(), "Không Tìm Thấy ", "Dialog", JOptionPane.ERROR_MESSAGE);
            }
        }
        else{
