...
public class HDCTVServiceIMpl implements HDCTService{
    ...
    public List<HDCTViewModel> getAll() {
        ...
        for (HDCTModel hdct : list) {
            listAl.add(new HDCTViewModel(hdct.getId(),hdct.getIdHD(),hdct.getIdSPCT(),hdct.getLoaiSanPham(),hdct.getThuongHieu(),hdct.getMauSac(),hdct.getKichCo(),hdct.getChatLieu(),hdct.getSoLuong(),hdct.getDonGia()));
        }
        return listAl;
    }
    ...
    public String getAdd(HDCTViewModel hdct) {
        HDCTModel hd=new HDCTModel(hdct.getId(),hdct.getIdHD(),hdct.getIdSPCT(),hdct.getLoaiSanPham(),hdct.getThuongHieu(),hdct.getMauSac(),hdct.getKichCo(),hdct.getChatLieu(),hdct.getSoLuong(),hdct.getDonGia());
    ...
    public List<GioHangViewModel> getGioHang() {
        ...
        for (GioHangModel gioHang : list) {
            listAll.add(new GioHangViewModel(gioHang.getId(),gioHang.getMaHD(),gioHang.getTenSp(),gioHang.getLoaiSanPham(),gioHang.getThuongHieu(),gioHang.getMauSac(),gioHang.getKichCo(),gioHang.getChatLieu(),gioHang.getSoLuong(),gioHang.getDonGia()));
